...
In this step, we'll create a new `js` file for the `taskManager` class, and add a new reference to it in the `index.html` file.

1. Create a `taskManager.js` file in the `js` folder
2. To index.html, add a `<script>` tag pointing to the `js/taskManager.js` file _before_ the `<script>` tag pointing to the `js/index.js` file.
...
> 3. `console.log()` the `tasks` property
>
> **Expected Result**
